"""`conventions/testing.md` §5.5 and §5.7, over the real ASGI path.

The seed set (ADR-029) is built to demonstrate `Accept-Language` filtering:
...
| Lirtuel | `fr`, `en` | the only language-scoped one, and every optional field the schema allows |

The seed is `demo/catalogs/` — Hadrien's three fixtures, minus the `self` links he authors for
the published output (ADR-030).
...
async def test_no_accept_language_returns_everything_recommended(
    ...
    assert await titles(client) == ["Librivox", "Lirtuel", "Project Gutenberg"]


async def test_french_keeps_the_french_catalog_and_drops_the_english_one(
    client: AsyncClient, seeded_catalogs: int
) -> None:
    assert await titles(client, "fr") == [*UNSCOPED, "Lirtuel"]


async def test_english_keeps_the_english_catalog_and_drops_the_french_one(
    client: AsyncClient, seeded_catalogs: int
) -> None:
    assert await titles(client, "en") == [*UNSCOPED, "Lirtuel"]
...
async def test_quality_orders_the_preferred_language_first(
    ...
    assert await titles(client, "en;q=0.8, fr;q=0.9") == [
        *UNSCOPED,  # scoped to no language, so relevant whatever was asked
        "Lirtuel",  # fr and en — ranked on the French, which was asked for more
    ]
...
async def test_reversing_the_qualities_reverses_the_order(
    ...
    assert await titles(client, "en;q=0.9, fr;q=0.8") == [*UNSCOPED, "Lirtuel"]
...
async def test_a_regional_request_still_finds_the_base_language(
    ...
    """
    assert await titles(client, "fr-BE") == [*UNSCOPED, "Lirtuel"]
...
async def test_header_order_is_the_preference_when_no_q_is_given(
    ...
    assert await titles(client, "fr, en") == [*UNSCOPED, "Lirtuel"]
...
async def test_a_regional_range_matches_a_plain_tag_in_our_data(
    ...
    assert await titles(client, "fr-FR") == [*UNSCOPED, "Lirtuel"]
...
async def test_no_internal_field_appears_in_any_response(
    ...
    """R3, asserted by key absence rather than by eye."""
    internal = {
        "id",
        "status",
        "recommended",
...
async def test_coverage_is_omitted_when_it_was_never_declared(
    ...
    """ADR-032 — emitting `global` here would assert worldwide reach nobody claimed.

    Lirtuel declares `subdivisions`, so this checks the field is *omitted* where it was never
    declared rather than absent from every response — which would also pass if the renderer
    dropped the field entirely.
    """
    by_title = {
        catalog["metadata"]["title"]: catalog["metadata"]
        for catalog in (await client.get("/")).json()["catalogs"]
    }
    assert by_title["Lirtuel"]["coverage"] == "subdivisions"
    for title, metadata in by_title.items():
        if title != "Lirtuel":
            assert "coverage" not in metadata


async def test_color_is_always_emitted(client: AsyncClient, seeded_catalogs: int) -> None:
    """Q7, leaning revised: Hadrien writes `"color": "gray"` explicitly in his own data."""
    for catalog in (await client.get("/")).json()["catalogs"]:
...
async def test_every_catalog_carries_a_synthesised_self_link_first(
    ...
    """ADR-030 — no seed record has a `self` link; the registry builds them."""
    for catalog in (await client.get("/")).json()["catalogs"]:
