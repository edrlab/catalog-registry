...
def build_catalog_document(feed: dict[str, Any], url: str, **editorial: Any) -> dict[str, Any]:
    ...
    *url* becomes the `catalog` rel, the operator asked for this address, and it is the
    ...
    # A remote feed's own metadata isn't this registry's identifier scheme, and there's no
    # human in the loop here to hand-assign one the way Hadrien does for the curated seed.
    # Q1: `identifier` is the upsert match key and is required on every catalog document.
...
async def add_catalog_from_url(url: str, document: dict[str, Any], settings: Settings) -> bool:
    ...
    *document*'s `identifier` was freshly generated by `build_catalog_document`, since a
    remote feed has no notion of this registry's identifier scheme. If this URL was `add`-ed
    before, that row's `identifier` is reused here instead, so a re-run updates it rather than
    inserting a duplicate under a new random identifier.
    """
    feed = {"metadata": {"title": f"Imported from {url}"}, "catalogs": [document]}
    engine = create_database_engine(settings)
    try:
        async with build_session_factory(engine)() as session:
            previously_added = await CatalogRepository(session).fetch_catalog_by_link_href(
                url, LinkRel.CATALOG
            )
            if previously_added is not None:
                document["metadata"]["identifier"] = previously_added.identifier
